...
AI API는 제미나이를 쓰기로 했다.


docs로 테스트를 해봤는데, 서버쪽 엔드 포인트는 어느 정도 된거 같다.

파일 업로드 -> 텍스트, 이미지 추출 -> 제미나이로 전송 -> 예상문제 및 답지, 번역본 등 제공.
텍스트, 이미지 추출 부분은 일단 어떻게든 될 것 같다.
png, txt로 업로드 후 저장이 됐으니까, 값을 받아오는 건 됐고,,,


이제 제미나이 api관련해서 좀 파봐야지.
